...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["Statistics"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), ["default"], "equals"))):
                            value += str("<CreateDir><Dir>{0}</Dir></CreateDir>").format(self.__params__.get("options").get("dest"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'folder_create'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'folder_create'))]/*[((local-name() = 'request'))]//*[((local-name() = 'CreateDir'))]/*[((local-name() = 'Dir'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'folder_create'))]/*[((local-name() = 'request'))]//*[((local-name() = 'CreateDir'))]/*[((local-name() = 'Dir'))]))")[i_item].text)
