...
class Action(base_action.Action):
    ...
    def __processrequest__(self):
        ...
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'store_config'))]/*[((local-name() = 'parsed'))]/*[((local-name() != 'file'))]))")):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((not(self.__strcmp__(item.tag, ["GeneratedPolicy"], "equals")))):
                        if((not(self.__strcmp__(part.tag, ["GeneratedPolicy"], "equals")))):
                            value += str("<{0} name='{1}'><mAdminState>disabled</mAdminState></{0}>").format(item.tag, item.attrib.get("name"))
            if((   (any((self.__strcmp__((item.tag, item.attrib.get("name")), [(part.tag, part.attrib.get("name"))], "equals")) for (i_part, part) in enumerate(item.getroottree().xpath("((./*[((local-name() = 'config_compare'))]/*[((local-name() = 'parsed'))]/*[((local-name() != 'file'))]))")))))):
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((   (self.__strcmp__(item.tag, ["GeneratedPolicy"], "equals")))):
                        if((not(self.__strcmp__(part.tag, ["GeneratedPolicy"], "equals")))):
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_disable'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'config_disable'))]/*[((local-name() = 'request'))]//*[((local-name() = 'modify-config'))]/*[((local-name() != 'file'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'config_disable'))]/*[((local-name() = 'request'))]//*[((local-name() = 'modify-config'))]/*[((local-name() != 'file'))]))")[i_item].attrib.get("name"))
