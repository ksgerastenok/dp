...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["file"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), [self.__params__.get("options").get("src")], "equals"))):
                            value += str("<MoveFile><sURL>{0}</sURL><dURL>{1}</dURL><Overwrite>on</Overwrite></MoveFile>").format(self.__params__.get("options").get("src"), self.__params__.get("options").get("dest"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'file_move'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'file_move'))]/*[((local-name() = 'request'))]//*[((local-name() = 'MoveFile'))]/*[((local-name() = 'sURL'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'file_move'))]/*[((local-name() = 'request'))]//*[((local-name() = 'MoveFile'))]/*[((local-name() = 'sURL'))]))")[i_item].text)
