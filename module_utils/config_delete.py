...
class Action(base_action.Action):
    ...
    def __processpolicy__(self):
        ...
        if((self.__strcmp__(type(self.__params__.get("options").get("policy")), [type(list())], "equals"))):
            for (i_item, item) in enumerate(self.__params__.get("options").get("policy")):
                if((self.__strcmp__(item.get("type"), ["delete"], "equals"))):
                    item["class"] = str(item.get("class"))
                    item["object"] = str(item.get("object"))
                    value += str("<{0} name='{1}'>{2}</{0}>").format(item.get("class"), item.get("object"), self.__yml2xml__(list()))
            ...
            for (i_types, types) in enumerate(self.__params__.get("options").get("policy").keys()):
                for (i_classes, classes) in enumerate(self.__params__.get("options").get("policy").get(types).keys()):
                    for (i_objects, objects) in enumerate(self.__params__.get("options").get("policy").get(types).get(classes).keys()):
                        if((self.__strcmp__(types, ["delete"], "equals"))):
                            classes = str(classes)
                            objects = str(objects)
                            value += str("<{0} name='{1}'>{2}</{0}>").format(classes, objects, self.__yml2xml__(list()))
    ...
    def __processrequest__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_delete'))]/*[((local-name() = 'policy'))]/*[((local-name() != 'file'))]))")):
            ...
            if((not(any((self.__strcmp__((item.tag, item.attrib.get("name")), [(part.tag, part.attrib.get("name"))], "equals")) for (i_part, part) in enumerate(item.getroottree().xpath("((./*[((local-name() = 'config_delete'))]/*[((local-name() = 'policy'))]/*[((local-name() != 'file'))]))")))))):
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((not(self.__strcmp__(item.tag, ["GeneratedPolicy"], "equals")))):
                        if((   (self.__strcmp__(part.tag, ["GeneratedPolicy"], "equals")))):
                            error += str(xml.tostring(part, method="xml", encoding="utf-8"))
        if((not(self.__strcmp__(value, [""], "equals")))):
            value = str("<dp:request domain='{0}' xmlns:dp='http://www.datapower.com/schemas/management'><dp:del-config>{1}</dp:del-config></dp:request>").format(self.__params__.get("connect").get("domain"), value)
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_delete'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'config_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'del-config'))]/*[((local-name() != 'file'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'config_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'del-config'))]/*[((local-name() != 'file'))]))")[i_item].attrib.get("name"))
