...
class Action(base_action.Action):
    ...
    def __processrequest__(self):
        ...
        if((   (self.__strcmp__(value, [""], "equals")))):
            value = str("<dp:request domain='{0}' xmlns:dp='http://www.datapower.com/schemas/management'><dp:get-filestore location='local:' layout-only='false'>{1}</dp:get-filestore></dp:request>").format(self.__params__.get("connect").get("domain"), value)
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'store_files'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'store_files'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'get-filestore'))]))")[i_item].tag[slice(49, 53, 1)])
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'store_files'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'get-filestore'))]))")[i_item].text)
            if((self.__strcmp__(xml.tostring(item, method="text", encoding="utf-8"), ["OK"], "equals"))):
                ...
                pass
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'store_files'))]/*[((local-name() = 'response'))]//*[((local-name() = 'filestore'))]//*[((local-name() = 'file'))]))")):
            item.tag = str(item.tag[slice(0, 4, 1)])
            item.attrib["name"] = str("{0}/{1}").format(item.getparent().attrib.get("name"), item.attrib.get("name")).replace("local:/", "local:///")
            if((not(self.__strcmp__(len(item.getchildren()), [0], "equals")))):
                value += str(xml.tostring(item, method="xml", encoding="utf-8"))
                pass
            else:
                error += str(xml.tostring(item, method="xml", encoding="utf-8"))
                pass
        if((not(self.__strcmp__(error, [""], "equals")))):
            raise(Exception(error))
