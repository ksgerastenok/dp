...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["Statistics"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), ["default"], "equals"))):
                            value += str("<RemoveCheckpoint><ChkName>{1}</ChkName></RemoveCheckpoint>").format(item.tag, item.attrib.get("name"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'check_delete'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'check_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'RemoveCheckpoint'))]/*[((local-name() = 'ChkName'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'check_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'RemoveCheckpoint'))]/*[((local-name() = 'ChkName'))]))")[i_item].text)
