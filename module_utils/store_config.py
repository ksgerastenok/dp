...
class Action(base_action.Action):
    ...
    def __processrequest__(self):
        ...
        if((   (self.__strcmp__(value, [""], "equals")))):
            value = str("<dp:request domain='{0}' xmlns:dp='http://www.datapower.com/schemas/management'><dp:get-config recursive='true' persisted='false'>{1}</dp:get-config></dp:request>").format(self.__params__.get("connect").get("domain"), value)
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'store_config'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'store_config'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'get-config'))]))")[i_item].tag[slice(49, 56, 1)])
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'store_config'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'get-config'))]))")[i_item].text)
            if((self.__strcmp__(xml.tostring(item, method="text", encoding="utf-8"), ["OK"], "equals"))):
                ...
                pass
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'store_config'))]/*[((local-name() = 'response'))]//*[((local-name() = 'config'))]/*[((local-name() != 'file'))]))")):
            item.tag = str(item.tag)
            item.attrib["name"] = str("{0}/{1}").format(item.getparent().attrib.get("name"), item.attrib.get("name")).replace("None/", "")
