	...
	switch(mode) {
		case String("LEXQH1"): {
			return String()
				...
				.concat("</MsgDesc>")
				.concat("</MQXQH>")
				.concat(ParseHDR(buffer.slice(buffer.length, buffer.length), mode));
		}
		case String("BEXQH1"): {
			return String()
				.concat("<MQXQH>")
		...
		case String("LERFH1"): {
			return String()
				...
				.concat(ParseHDR(buffer.slice(32, buffer.length), mode.concat("X")))
				.concat("</NameValueString>")
				.concat("</MQRFH>")
				.concat(ParseHDR(buffer.slice(buffer.length, buffer.length), mode));
		}
		...
		case String("LERFH2"): {
			return String()
				...
				.concat(ParseHDR(buffer.slice(36, buffer.length), mode.concat("X")))
				.concat("</NameValueData>")
				.concat("</MQRFH2>")
				.concat(ParseHDR(buffer.slice(buffer.length, buffer.length), mode));
		}
		...
		case String("LERFH1X"): {
			return String()
				...
				.concat("</Value>")
				.concat("</NameValue>")
				.concat(ParseHDR(buffer.slice(buffer.toString("utf-8").split(" ")[0].length + 1 + buffer.toString("utf-8").split(" ")[1].length + 1, buffer.length), mode));
		}
		case String("BERFH1X"): {
			return String()
				.concat("<NameValue>")
		...
		case String("LERFH2X"): {
			return String()
				...
				.concat("</NameValue>")
				.concat(ParseHDR(buffer.slice(4 + buffer.slice(0, 4).readInt32LE(), buffer.length), mode));
		}
		case String("BERFH2X"): {
			return String()
				.concat("<NameValue>")
				...
				.concat("</NameValue>")
				.concat(ParseHDR(buffer.slice(4 + buffer.slice(0, 4).readInt32BE(), buffer.length), mode));
		}
	}
...
function ParseBUF(buffer) {
	...
	switch(buffer.slice(0, 8).toString("hex")) {
		case String("5851482001000000"): {
			require("header-metadata").current.set("MQXQH", ParseHDR(buffer.slice(0, 428), "LEXQH1"));
			return ParseBUF(buffer.slice(428, buffer.length));
		}
		case String("5851482000000001"): {
			require("header-metadata").current.set("MQXQH", ParseHDR(buffer.slice(0, 428), "BEXQH1"));
			return ParseBUF(buffer.slice(428, buffer.length));
		}
		case String("5851482002000000"): {
			require("header-metadata").current.set("MQXQH", ParseHDR(buffer.slice(0, 468), "LEXQH2"));
			return ParseBUF(buffer.slice(468, buffer.length));
		}
		case String("5851482000000002"): {
			require("header-metadata").current.set("MQXQH", ParseHDR(buffer.slice(0, 468), "BEXQH2"));
			return ParseBUF(buffer.slice(468, buffer.length));
		}
		case String("4d44452001000000"): {
			require("header-metadata").current.set("MQMDE", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32LE()), "LEMDE1"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32LE(), buffer.length));
		}
		case String("4d44452000000001"): {
			require("header-metadata").current.set("MQMDE", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32BE()), "BEMDE1"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32BE(), buffer.length));
		}
		case String("4d44452002000000"): {
			require("header-metadata").current.set("MQMDE", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32LE()), "LEMDE2"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32LE(), buffer.length));
		}
		case String("4d44452000000002"): {
			require("header-metadata").current.set("MQMDE", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32BE()), "BEMDE2"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32BE(), buffer.length));
		}
		case String("5246482001000000"): {
			require("header-metadata").current.set("MQRFH", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32LE()), "LERFH1"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32LE(), buffer.length));
		}
		case String("5246482000000001"): {
			require("header-metadata").current.set("MQRFH", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32BE()), "BERFH1"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32BE(), buffer.length));
		}
		case String("5246482002000000"): {
			require("header-metadata").current.set("MQRFH2", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32LE()), "LERFH2"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32LE(), buffer.length));
		}
		case String("5246482000000002"): {
			require("header-metadata").current.set("MQRFH2", ParseHDR(buffer.slice(0, buffer.slice(8, 12).readInt32BE()), "BERFH2"));
			return ParseBUF(buffer.slice(buffer.slice(8, 12).readInt32BE(), buffer.length));
		}
	}
...

{
	session.input.readAsBuffer(
		function (error, buffer) {
			...
			session.output.write(ParseBUF(buffer));
		}
	);
}
