	...
	switch (mode) {
		...
		case String("LERFH1X"): {
			return String()
				.concat("<NameValue>")
				.concat("<Name>")
				.concat(buffer.slice(0, buffer.length).toString("utf-8").split(" ")[0])
				.concat("</Name>")
				.concat("<Value>")
				.concat(buffer.slice(0, buffer.length).toString("utf-8").split(" ")[1])
				.concat("</Value>")
				.concat("</NameValue>")
				.concat(parseHeader(buffer.slice(buffer.slice(0, buffer.length).toString("utf-8").split(" ")[0].length + 1 + buffer.slice(0, buffer.length).toString("utf-8").split(" ")[1].length + 1, buffer.length), mode));
		}
		case String("BERFH1X"): {
			return String()
				.concat("<NameValue>")
