			...
			<xsl:call-template name="gw:HeaderWrite">
				<xsl:with-param name="pHeader">
					<xsl:element name="header">
						...
						<xsl:value-of select="normalize-space('IRUS.DEPLOY.GW1')"/>
					</xsl:element>
					<xsl:element name="header">
						<xsl:attribute name="name">
							<xsl:value-of select="normalize-space('ReplyToQ')"/>
						</xsl:attribute>
						<xsl:value-of select="normalize-space($vMQMD/header/MQMD/ReplyToQ/text())"/>
					</xsl:element>
				</xsl:with-param>
					...
					<xsl:element name="header">
						...
						<xsl:element name="MQMD">
							...
							<xsl:element name="MsgId">
								<xsl:value-of select="normalize-space($vMQMD/header/MQMD/MsgId/text())"/>
							</xsl:element>
							<xsl:element name="CorrelId">
								<xsl:value-of select="normalize-space($vMQMD/header/MQMD/CorrelId/text())"/>
							</xsl:element>
							<xsl:element name="ReplyToQMgr">
								<xsl:value-of select="normalize-space($vMQXQH/header/MQXQH/RemoteQMgrName/text())"/>
							</xsl:element>
					...
					<xsl:element name="header">
						...
						<xsl:element name="MQOD">
							...
							<xsl:element name="ObjectQMgrName">
								<xsl:value-of select="normalize-space($vMQMD/header/MQMD/ReplyToQMgr/text())"/>
							</xsl:element>
