...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="/">
		...
		<xsl:variable name="vURL">
			<xsl:value-of select="normalize-space('dpmq://')"/>
			<xsl:value-of select="normalize-space($vMQXQH/header/MQXQH/RemoteQMgrName/text())"/>
			<xsl:value-of select="normalize-space('/?RequestQueue=')"/>
			<xsl:value-of select="normalize-space($vMQXQH/header/MQXQH/RemoteQName/text())"/>
		</xsl:variable>
		<xsl:variable name="vDebug">
			<dp:set-variable name="'var://service/routing-url'" value="string($vURL/text())"/>
		</xsl:variable>
