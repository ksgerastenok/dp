...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["file"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), [self.__params__.get("options").get("dest")], "equals"))):
                            value += str("<DeleteFile><File>{0}</File></DeleteFile>").format(self.__params__.get("options").get("dest"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'file_delete'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'file_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'DeleteFile'))]/*[((local-name() = 'File'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'file_delete'))]/*[((local-name() = 'request'))]//*[((local-name() = 'DeleteFile'))]/*[((local-name() = 'File'))]))")[i_item].text)
