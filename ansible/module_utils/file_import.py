...
class Action(base_action.Action):
    ...
    def __processrequest__(self):
            ...
            if((   (any((self.__strcmp__((item.tag, item.attrib.get("name")), [(part.tag, part.attrib.get("name"))], "equals")) for (i_part, part) in enumerate(item.getroottree().xpath("((./*[((local-name() = 'store_files'))]/*[((local-name() = 'parsed'))]/*[((local-name() = 'file'))]))")))))):
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["file"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), [self.__params__.get("options").get("dest")], "equals"))):
                            value += str()
        ...
        if((self.__strcmp__(value, [""], "equals"))):
            value = str("<dp:request domain='{0}' xmlns:dp='http://www.datapower.com/schemas/management'><dp:set-file name='{1}'>{2}</dp:set-file></dp:request>").format(self.__params__.get("connect").get("domain"), self.__params__.get("options").get("dest"), base64.b64encode(open(self.__params__.get("options").get("src"), "rb").read()))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'file_import'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'file_import'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'set-file'))]))")[i_item].tag[slice(49, 53, 1)])
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'file_import'))]/*[((local-name() = 'request'))]//*[((local-name() = 'request'))]/*[((local-name() = 'set-file'))]))")[i_item].attrib.get("name"))
            if((self.__strcmp__(xml.tostring(item, method="text", encoding="utf-8"), ["OK"], "equals"))):
                ...
                pass
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'file_import'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'file'))]))")):
