...
class Action(base_action.Action):
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_import'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str("file")
            ...
            if((self.__strcmp__(xml.tostring(item, method="text", encoding="utf-8"), ["OK"], "equals"))):
                ...
                pass
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_import'))]/*[((local-name() = 'response'))]//*[((local-name() = 'import'))]//*[((local-name() = 'file-result'))]))")):
            item.tag = str(item.tag[slice(0, 4, 1)])
            item.attrib["name"] = str(item.attrib.get("name"))
            if((not(self.__strcmp__(item.attrib.get("result"), ["ERROR"], "equals")))):
                value += str("<{0} name='{1}'>{2}</{0}>").format(item.tag, item.attrib.get("name"), str("<status>{0}</status><reason>{1}</reason>").format(item.attrib.get("result"), xml.tostring(item, method="text", encoding="utf-8")))
                pass
            else:
                error += str("<{0} name='{1}'>{2}</{0}>").format(item.tag, item.attrib.get("name"), str("<status>{0}</status><reason>{1}</reason>").format(item.attrib.get("result"), xml.tostring(item, method="text", encoding="utf-8")))
                pass
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'config_import'))]/*[((local-name() = 'response'))]//*[((local-name() = 'import'))]//*[((local-name() = 'cfg-result'))]))")):
            item.tag = str(item.attrib.get("class"))
            item.attrib["name"] = str(item.attrib.get("name"))
            if((not(self.__strcmp__(item.attrib.get("status"), ["ERROR"], "equals")))):
