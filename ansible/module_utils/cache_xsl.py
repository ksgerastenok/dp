...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["XMLManager"], "equals"))):
                        if((self.__strcmp__(part.tag, ["XMLManager"], "equals"))):
                            value += str("<FlushStylesheetCache><XMLManager>{1}</XMLManager></FlushStylesheetCache>").format(item.tag, item.attrib.get("name"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'cache_xsl'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'cache_xsl'))]/*[((local-name() = 'request'))]//*[((local-name() = 'FlushStylesheetCache'))]/*[((local-name() = 'XMLManager'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'cache_xsl'))]/*[((local-name() = 'request'))]//*[((local-name() = 'FlushStylesheetCache'))]/*[((local-name() = 'XMLManager'))]))")[i_item].text)
