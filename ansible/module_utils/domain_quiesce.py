...
class Action(base_action.Action):
                ...
                for (i_part, part) in enumerate(item.xpath("((./.))")):
                    if((self.__strcmp__(item.tag, ["Statistics"], "equals"))):
                        if((self.__strcmp__(part.attrib.get("name"), ["default"], "equals"))):
                            value += str("<DomainQuiesce><name>{0}</name><timeout>60</timeout></DomainQuiesce>").format(self.__params__.get("connect").get("domain"))
    ...
    def __processresponse__(self):
        ...
        error = value = str()
        for (i_item, item) in enumerate(xml.fromstring(self.__getxml__(False)).xpath("((./*[((local-name() = 'domain_quiesce'))]/*[((local-name() = 'response'))]//*[((local-name() = 'response'))]/*[((local-name() = 'result'))]))")):
            item.tag = str(item.getroottree().xpath("((./*[((local-name() = 'domain_quiesce'))]/*[((local-name() = 'request'))]//*[((local-name() = 'DomainQuiesce'))]/*[((local-name() = 'name'))]))")[i_item].tag)
            item.attrib["name"] = str(item.getroottree().xpath("((./*[((local-name() = 'domain_quiesce'))]/*[((local-name() = 'request'))]//*[((local-name() = 'DomainQuiesce'))]/*[((local-name() = 'name'))]))")[i_item].text)
